...
TAG_BRANCH="$(echo $BRANCH | sed -e 's#r/#rc-#')"
...
transforms["BASE_IMAGE_TAG_PREFIX=\"$SRC_BRANCH-"]="BASE_IMAGE_TAG_PREFIX=\"$TAG_BRANCH-"
# undo one transform
transforms["if \[ \"\$BRANCH\" == \"${BRANCH}\" \]; then"]="if [ \"\$BRANCH\" == \"${SRC_BRANCH}\" ]; then"
...
for c in $(find "${jobs[@]}" -name config.xml); do 
   ...
   mkdir -p "$d2"
   cp "$c" "$c2"
   for k in "${jobs[@]}" "${workdirs[@]}" "starlingx/$SRC_BRANCH/debian" "TAG=$SRC_BRANCH" "$SRC_BRANCH" "if \[ \"\$BRANCH\" == \"${BRANCH}\" \]; then"; do
...
for c in $(find "${workdirs[@]}" -maxdepth 1 -name build.conf); do
   ...
   mkdir -p "$d2"
   cp "$c" "$c2"
   for k in "BASE_IMAGE_TAG_PREFIX=\"$SRC_BRANCH-" "${jobs[@]}" "${workdirs[@]}" "starlingx/$SRC_BRANCH/debian" "TAG=$SRC_BRANCH" "$SRC_BRANCH"; do
       sed -i "s#$k#${transforms["$k"]}#g" "$c2"
   done
done
