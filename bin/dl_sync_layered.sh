...
if [ -f "$DAILY_DL_SYNC_DIR/stx_tool_utils.sh" ]; then
    source "$DAILY_DL_SYNC_DIR/stx_tool_utils_layered.sh"
...
while getopts "b:d:h" opt; do
    case "${opt}" in
        ...
        d)
            ...
            STX_BRANCH_ROOT_DIR="${OPTARG}"
            if [ "$STX_BRANCH_ROOT_DIR" == "" ]; then
                usage
                exit 1
            fi
            ;;
...
STX_DL_ROOT_DIR="$STX_BRANCH_ROOT_DIR/$STX_BRANCH"
    ...
    grep -v '^#' $FILE | while IFS='#' read -r TAR DIR URL METHOD UTIL SCRIPT; do
        ...
        if [[ "$TAR" =~ ^'!' ]]; then
            case $METHOD in
                ...
                git|git_script)
                    BRANCH="$UTIL"
                    dl_bare_git_from_url "$URL" "$BRANCH" ""
                    if [ $? -ne 0 ]; then
                        echo "Error: Failed to download '$URL' while processing '$TARBALL_NAME'"
                        ERR_COUNT=$((ERR_COUNT+1))
                    fi
...
stx_clone_or_update "$STX_TOOLS_BRANCH" "$STX_DL_ROOT_DIR"
if [ $? -ne 0 ]; then
...
RPM_LST_FILES=$(grep -l '://' $STX_DL_ROOT_DIR/cgcs-root/stx/*/centos_*rpms*.lst; grep -l '://' $STX_DL_ROOT_DIR/stx-tools/centos-mirror-tools/config/centos/*/rpms*.lst)

# At time of writing, only expect tarball-dl.lst
NON_RPM_FILES=$((grep -l '://' $STX_DL_ROOT_DIR/cgcs-root/stx/*/centos_*.lst; grep -l '://' $STX_DL_ROOT_DIR/stx-tools/centos-mirror-tools/config/centos/*/*.lst) | grep -v '[/]rpms[^/]*$')
